...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   *
   * This is the only place a credential is minted. Every provider gets the
   * thread-scoped board capability; preview is added only when browser access
   * is enabled. The `/mcp` endpoint accepts only tokens issued here.
   */
  /**
   ...
   * optional preview capability is attached. Denying is the safe direction:
   * board access remains available, while an explicit browser "off" can never
   * silently become "on".
   */
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      const credential = yield* issueMcpCredential({
        threadId,
        providerInstanceId,
        capabilities: new Set(browserEnabled ? ["board", "preview"] : ["board"]),
      });
      if (credential) {
